 ...
 * @author rdyer
 */
public class ECJTreeAdapter extends ASTVisitor {
	private HashMap<String, Integer> nameIndices;

	private CompilationUnit root = null;
	private Ast.PositionInfo.Builder pos = null;
	private String src = null;

	private Ast.Namespace.Builder b = Ast.Namespace.newBuilder();
	private List<Ast.Comment> comments = new ArrayList<>();
	private List<String> imports = new ArrayList<>();
	private Stack<List<Ast.Declaration>> declarations = new Stack<>();
	private Stack<Ast.Modifier> modifiers = new Stack<>();
	private Stack<Ast.Expression> expressions = new Stack<>();
	private Stack<List<Ast.Variable>> fields = new Stack<>();
	private Stack<List<Ast.Method>> methods = new Stack<>();
	private Stack<List<Ast.Statement>> statements = new Stack<>();

	public ECJTreeAdapter(String src, HashMap<String, Integer> nameIndices) {
		super();
		this.src = src;
		this.nameIndices = nameIndices;
	}

	public Ast.Namespace getNamespaces(CompilationUnit node) {
		root = node;
		node.accept(this);
		return b.build();
	}

	public List<Ast.Comment> getComments() {
		return comments;
	}
...
*/
	...
	private void buildPosition(final ASTNode node) {
		pos = Ast.PositionInfo.newBuilder();
...
//		b.setPosition(pos.build());
		...
		for (Object t : node.types()) {
			declarations.push(new ArrayList<>());
...
//		b.setPosition(pos.build());
	...
	public boolean visit(MethodDeclaration node) {
		List<Ast.Method> list = methods.peek();
		Ast.Method.Builder b = Ast.Method.newBuilder();
//		b.setPosition(pos.build());
		...
		for (Object o : node.parameters()) {
			...
			Ast.Variable.Builder vb = Ast.Variable.newBuilder();
//			vb.setPosition(pos.build()); // FIXME
		...
		if (node.getBody() != null) {
			statements.push(new ArrayList<Ast.Statement>());
...
//		b.setPosition(pos.build());
	...
	public boolean visit(FieldDeclaration node) {
		List<Ast.Variable> list = fields.peek();
		for (Object o : node.fragments()) {
			VariableDeclarationFragment f = (VariableDeclarationFragment)o;
			Ast.Variable.Builder b = Ast.Variable.newBuilder();
...
//		b.setPosition(pos.build());
		List<Ast.Statement> list = statements.peek();
